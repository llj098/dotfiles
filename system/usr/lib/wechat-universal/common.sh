...
try_start() {
    ...
    # linux-setup: native Wayland transport (keep the packaged sandbox).
    local -a BWRAP_WAYLAND_BINDS=()
    local wayland_socket="${WAYLAND_DISPLAY:-}"
    export QT_QPA_PLATFORM="${QT_QPA_PLATFORM:-${WAYLAND_DISPLAY:+wayland;xcb}}"
    QT_QPA_PLATFORM="${QT_QPA_PLATFORM:-xcb}"
    if [[ "$QT_QPA_PLATFORM" == wayland* ]]; then
        [[ "$wayland_socket" == /* ]] || wayland_socket="${XDG_RUNTIME_DIR%/}/$wayland_socket"
        [[ -n "${WAYLAND_DISPLAY:-}" && -S "$wayland_socket" ]] || {
            echo "Error: Wayland socket is unavailable: $wayland_socket" >&2
            return 1
        }
        BWRAP_WAYLAND_BINDS=(--ro-bind "$wayland_socket" "$wayland_socket")
    fi
    export QT_AUTO_SCREEN_SCALE_FACTOR=1 \
        PATH="/sandbox:${PATH}"

    if [[ -z "${WECHAT_IME_WORKAROUND}" || "${WECHAT_IME_WORKAROUND}" == 'auto' ]]; then
        case "${QT_QPA_PLATFORM}:${XMODIFIERS}" in
            wayland*:*)
                WECHAT_IME_WORKAROUND='wayland'
                ;;
            *:*@im=fcitx*)
    ...
    case "${WECHAT_IME_WORKAROUND}" in
        wayland)
            # WeChat's bundled Qt needs explicit v3 selection on Hyprland.
            unset GTK_IM_MODULE QT_IM_MODULES
    ...
    *)
    ...
    exec bwrap "${BWRAP_ARGS[@]}" "${BWRAP_CUSTOM_BINDS[@]}" "${BWRAP_WAYLAND_BINDS[@]}" "${BWRAP_DEV_BINDS[@]}" /opt/wechat-universal/wechat "$@"
        ...
        '--help')
            if [[ "${LANG}" == zh_CN* ]]; then
                ...
                printf '    --%s\t%s\n' \
                    ...
                    'ime [输入法名称或特殊值]' '应用输入法对应环境变量修改，可支持：wayland (内置text-input-v3), fcitx (不论是否为5), ibus，特殊值：none不应用，auto在Wayland下使用v3，否则按XMODIFIERS判断。默认: auto；环境变量: WECHAT_IME_WORKAROUND'\
                ...
                printf '    --%s\t%s\n' \
                    ...
                    'ime [input method]' 'Apply IME-specific workaround, support: wayland (bundled text-input-v3), fcitx (also for 5), ibus, default: auto (v3 on Wayland), as environment: WECHAT_IME_WORKAROUND'\
